...
use crush_circuit::sha2::Sha2CpuProverExt;
...
impl OpenVmISA for CrushISA {
    ...
    fn create_dummy_airs<E: VmCircuitExtension<BabyBearSC>>(
        ...
        if let Some(sha2) = &config.sha2 {
            VmCircuitExtension::extend_circuit(sha2, &mut inventory)?;
        }
        Ok(inventory)
    }
    ...
    fn create_dummy_chip_complex_cpu(
        ...
        // SHA-2 opcodes are not in `allowed_opcodes`, so they never appear inside an
        // autoprecompile and these chips are never asked for a trace. They are still built
        // to keep the chip list index-aligned with `create_dummy_airs`, which
