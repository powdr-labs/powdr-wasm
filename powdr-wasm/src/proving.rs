...
///
/// Takes the config rather than defaulting it: with an extension enabled the AIR set
/// and the executor inventory both differ, so a default-built config would be missing
/// the extension's executors.
...
/// generated without an extension cannot prove a program that uses it.
...
pub fn prove(
    ...
    let sdk = if apc_count == 0 {
        // The compiled config carries whatever extensions were requested (e.g. --sha2),
        // so it is what a freshly-built SDK must be keyed on.
...
{
    // Use the cached key for the default config, a fresh one when extensions differ.
    // pk_storage keeps the owned key alive for the rest of the call.
    let pk_storage = if vm_config.sha2.is_some() {
